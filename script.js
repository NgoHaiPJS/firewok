...
// --- Extra UI wiring: screenshot, shortcuts, tiny feedback sound ---
function takeScreenshot() {
	...
	try {
		...
		// black background for nicer image
		ctx.fillStyle = '#000';
		ctx.fillRect(0,0,tmp.width,tmp.height);
		ctx.drawImage(canvas, 0, 0);
		const data = tmp.toDataURL('image/png');
		window.open(data, '_blank');
		// small feedback beep if audio allowed
		tryPlayBeep();
	}
	catch(e) {
...
document.addEventListener('DOMContentLoaded', () => {
	...
	if (shot) shot.addEventListener('click', takeScreenshot);
